...
#pragma warning restore CS1998
    ...
    private IFeatureDefinition<IConfiguration> ReadFeatureDefinition(IConfigurationSection configurationSection)
    {
      ...
      */
      ...
      var enabledFor = new List<FeatureFilterEnabledFor<IConfiguration>>();
      ...
      else
      {
        ...
        foreach (IConfigurationSection section in filterSections)
        {
          ...
          if (int.TryParse(section.Key, out int i) && !string.IsNullOrEmpty(section[nameof(FeatureFilterEnabledFor<IConfiguration>.Name)]))
          {
            enabledFor.Add(new FeatureFilterEnabledFor<IConfiguration>()
            {
              Name = section[nameof(FeatureFilterEnabledFor<IConfiguration>.Name)],
              Parameters = new ConfigurationWrapper(section.GetSection(nameof(FeatureFilterEnabledFor<IConfiguration>.Parameters)))
            });
          }
        }
      }
